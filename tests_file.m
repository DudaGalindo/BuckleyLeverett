...
solver = 'MUSCL';
[Sw8_MUSCL] = main_solver(M, phi, mi_o, mi_w, Sw, Swc, Sor, n_o, n_w, t, t_final, solver);
...
solver = 'MUSCL';
[Sw16_MUSCL] = main_solver(M, phi, mi_o, mi_w, Sw, Swc, Sor, n_o, n_w, t, t_final, solver);
...
solver = 'MUSCL';
[Sw32_MUSCL] = main_solver(M, phi, mi_o, mi_w, Sw, Swc, Sor, n_o, n_w, t, t_final, solver);
...
solver = 'MUSCL';
[Sw64_MUSCL] = main_solver(M, phi, mi_o, mi_w, Sw, Swc, Sor, n_o, n_w, t, t_final, solver);
...
solver = 'MUSCL';
[Sw128_MUSCL] = main_solver(M, phi, mi_o, mi_w, Sw, Swc, Sor, n_o, n_w, t, t_final, solver);
...
solver = 'MUSCL';
[Sw256_MUSCL] = main_solver(M, phi, mi_o, mi_w, Sw, Swc, Sor, n_o, n_w, t, t_final, solver);
...
solver = 'MUSCL';
[Sw512_MUSCL] = main_solver(M, phi, mi_o, mi_w, Sw, Swc, Sor, n_o, n_w, t, t_final, solver);
...
solver = 'MUSCL';
[Sw1024_MUSCL] = main_solver(M, phi, mi_o, mi_w, Sw, Swc, Sor, n_o, n_w, t, t_final, solver);
...
fig2 = figure(2);
 set(fig2, 'color', 'w')
 hold on
 title('Water Saturation Profile - MUSCL')
 xlabel('Distance')
 ylabel('Water Saturation')
 plot(x8,Sw8_MUSCL,'r')
 hold on
 plot(x16, Sw16_MUSCL, 'k')
 plot(x32, Sw32_MUSCL, 'b')
 plot(x64, Sw64_MUSCL, 'm')
 plot(x128, Sw128_MUSCL, 'Color', [0.01 0.012 0.782])
 plot(x256, Sw256_MUSCL, 'Color', [0.4 0.15 0.2])
 plot(x512, Sw512_MUSCL, 'Color', [0.02 0.8 0.2])
 plot(x1024, Sw1024_MUSCL, 'Color', [0.67 0.6 0.07])
 hold on
 plot(Xsw(:,nts), Swt, '-r', 'LineWidth', 2.0);
 hold on
 plot([0 Xsw(1,nts)], [1-Sor 1-Sor], '-r', 'LineWidth', 2.0);
